<?php
/**
 * Buttonizer API request handler.
 *
 * Handles external API communication, token management, and error handling.
 *
...
class ApiRequest
{
    ...
    public static function saveApiToken(string $token)
    {
        ...
        register_setting($name, $name . '_site_connection', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_text_field'
        ]);
        update_option($name . '_site_connection', $token);
    ...
    public static function getApiToken()
    {
        ...
        // Connect Site ID
        $token = get_option($name . '_site_connection');

        // Token not available. Get from transient instead
        if (!$token) {
            $token = get_transient($name . '_site_connection');
            ...
            delete_transient($name . '_site_connection');
        }
    ...
    public static function deleteApiToken()
    {
        ...
        delete_option($name . '_site_connection');
    ...
    public static function post(string $path = '/', array $body = [], array $headers = [], $withAuthorization = true, $refreshtokenCheck = true)
    {
        ...
        $args = [
            'body' => $body,
            'headers' => $headers,
            "timeout" => PluginConfig::get('api_timeout', 20)
        ];

        // Execute
        $response = wp_remote_post(PluginConfig::apiUri() . ($withAuthorization ? '/external' : '') . $path, $args);
